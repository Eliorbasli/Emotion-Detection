...
# Main function for analysis
    ...
    def detect_face(self, frame):
        # painting the frame in grey colors
        ...
        for face in faces:
            ...
            #location of the left eye according to shape_predictor_68_face_landmarks.dat
            ...
            cv2.putText(frame, emotions[self.emotion],
                        (10, 100), font, 2, (0, 0, 220), 3)
            cv2.putText(frame, ci,
                        (50, 250), font, 2, (0, 0, 200), 3)
...
# Function for detecting emotion
    ...
    def detect_emotion(self, gray):
        ...
        # check if the face array is not empty --> the camera detected a face
        if len(faces) > 0:
...
# Downleft	2	1.5	0
        ...
        if self.size < 0.2:
            gaze_weights = 0
            # eyes are not fully open - Semi open
        elif self.size > 0.2 and self.size < 0.3:
            gaze_weights = 1.5
        else:
            # eyes are open in normal size
            if self.x < 2 and self.x > 1:
                # eyes are centerd and "looking into the camera"
                gaze_weights = 5
            else:
                gaze_weights = 2
